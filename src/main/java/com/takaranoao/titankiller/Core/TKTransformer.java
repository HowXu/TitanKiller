        ...
        ClassNode craft = getClassNode(basicClass);
            ...
            for (AbstractInsnNode node : methodNode.instructions.toArray()) {
                if (node.getOpcode() == 180 && ("loadedEntityList".equals(((FieldInsnNode)node).name) || "field_72996_f".equals(((FieldInsnNode)node).name))) {
                    methodNode.instructions.insert(node, new MethodInsnNode(184, "com/takaranoao/titankiller/Core/Hook", "hook", "(Ljava/util/List;)Ljava/util/List;", false));
                    break label94;
                    }
                }
            }
            ...
            if ("func_70106_y".equals(methodNode.name)) {
                for (AbstractInsnNode node : methodNode.instructions.toArray()) {
                    if (node.getOpcode() == 182) {
                        methodNode.instructions.insert(node, new InsnNode(2));
                        methodNode.instructions.insert(node, new InsnNode(87));
                        break label91;
                    }
                }
            }
